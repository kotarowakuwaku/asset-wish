...
- `docs/spec-changes.md` — **使ってみて出た仕様変更。進行中。** 残り1本の計画と、着手前に決めるべき論点がある
...
2. **月次支出とウィッシュ由来の確定支出を二重計上しない。** 生活費等は入出金の明細（`transactions` の `adjustment`）に、ライブや旅行などは `wishes` に登録する。同じ支出を両方に入れる実装・UI を作らない。**月次の集計に足すのも手入力の明細だけ**で、`wish_paid` は除く（2026-08-01 に月次収支の手入力を廃止し、明細からの集計に置き換えた）。`monthly_balances` は明細を打ち始める前の記録として読み取り専用で残っており、**明細が1件も無い月に限って**集計に使う。同じ月について明細と手入力の両方を足さない。
...
| 項目 | 決定 | 理由 |
| --- | --- | --- |
| 画面の切り替え | ライブラリを入れず hash で持つ（`src/app/router.ts`） | 画面は6つ、入れ子も動的な経路も無い |
