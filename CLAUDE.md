...
**立替（貸し借り）は実質資産に含めない。** 別枠の参考値として表示するだけ。カードで立て替えた場合は現金がまだ出ていないため、「立て替えた ＝ 現金が出た」と決め打ちできない、というのが理由（2026-07-30 の仕様変更）。
...
4. **立替は実質資産に加算しない。** 未回収残高は `amount - collected_amount` で、**投資資産と同じ別枠の参考値**として返す。立替の登録・回収で口座残高を動かしてはならず、取引履歴も作らない。回収額が未回収残高を超える操作は必ずエラーにする。
