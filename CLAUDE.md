...
- `docs/spec-changes.md` — **使ってみて出た仕様変更。5本すべて完了。** 決定とその理由、見送った選択肢が残っている
...
2. **月次支出とウィッシュ由来の確定支出を二重計上しない。** 生活費等は入出金の明細（`transactions` の `adjustment`）に、ライブや旅行などは `wishes` に登録する。同じ支出を両方に入れる実装・UI を作らない。**月次の集計に足すのは口座の入出金そのものを表す種別だけ**（手入力の明細 `adjustment` と定期入出金の適用 `recurring_applied`）で、`wish_paid` は除く（2026-08-01 に月次収支の手入力を廃止し、明細からの集計に置き換えた）。`monthly_balances` は明細を打ち始める前の記録として読み取り専用で残っており、**明細が1件も無い月に限って**集計に使う。同じ月について明細と手入力の両方を足さない。
...
| `recurring_entries` | `updateRecurringAppliedStatement`（適用済み年月のみ） | `amount` / `day_of_month` / `account_id` を書ける文 |
| `monthly_balances` | 無し（読み取り専用） | すべて。明細から集計する形に変えた |
