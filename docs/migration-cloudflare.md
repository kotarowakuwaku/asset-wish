...
> **名前について（2026-07-30）：** 本書の SQL 例に出てくる `lendings` テーブルと `collected_amount` カラムは、その後 **`loans` / `settled_amount` に改名した**（`migrations/0002_loans.sql`）。**本書は当時の名前のまま残す。** 実測した内容の記録であり、書き換えると「何を試したときの結果か」がずれる。改名の経緯は `docs/spec-changes.md` の 2b にある。4章で述べている D1 の癖と、そこから導いた楽観ロックの形は、名前が変わっても**そのまま有効**。
