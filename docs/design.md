...
CREATE TABLE transactions (
    ...
    occurred_on TEXT    NOT NULL
        CHECK (occurred_on GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]'),
    -- 手入力の明細の覚書。分類（カテゴリ）は持たない（0003 で追加）。
...
もともとは「残高の手動調整」のつもりで置いていたが、それを作る経路が無く1件も存在しなかった。入出金の明細を打てるようにした際、**この種別に「手入力の明細」という意味を与えた**（2026-08-01）。新しい種別を足さなかったのは、実体の無い種別が2つ並ぶのを避けるため。

`POST /api/transactions` で作れるのはこの種別だけで、**削除できるのもこの種別だけ。** 参照先を持つ履歴（`wish_paid` など）は対になる記録があり、履歴だけ消すとウィッシュの状態と食い違う。判定は `Transaction.ensureDeletable()` が持ち、違反は 422（`TRANSACTION_NOT_DELETABLE`）。
...
入出金の明細を1件打つ。**口座残高が同額だけ動き、履歴が残る。** 貸し借り（口座を触らない）とはここが違う。残高の更新と履歴の記録は1回の `db.batch()` に載る（不変条件10）。

**金額は符号付き。** 出金は負、入金は正。`direction` のような項目を別に置かないのは、`Transaction.amount` がもともと符号付きで確定しており、`GET /api/transactions` も符号付きで返しているため。入口だけ形を変えると、同じ値の表現が2つになる。0 は 422（`INVALID_AMOUNT`）。残高が動かない記録に意味は無い。

**`kind` は受け取らない。** ここで作れるのは手入力の明細（`adjustment`）だけで、`wish_paid` などはそれぞれの経路が作る。送ると 400。

**`DELETE /api/transactions/{id}`**

明細を消し、動かした分の残高を戻す。**消せるのは手入力の明細だけ**で、それ以外は 422（`TRANSACTION_NOT_DELETABLE`）。
